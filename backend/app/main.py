...
app.add_middleware(
    ...
    allow_origin_regex=r"https://footcall-frontend-.*-danii-26-s-projects\.vercel\.app",
...
# HU-14 - Obtener reservas de un cliente desde la base de datos
@app.get("/api/cliente/reservas", response_model=list)
def obtener_reservas_cliente(
    current_user: dict = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """
    HU-14: Obtiene el historial de reservas de un cliente,
    ordenado de la más reciente a la más antigua.
    """
    try:
        usuario_id = current_user.get("id")
        
        # Si el usuario_id es un string (e.g. UUID de Supabase), buscamos su ID numérico en usuarios
        if isinstance(usuario_id, str):
            query_usuario = text("SELECT id FROM usuarios WHERE email = :email AND eliminado = FALSE")
            usuario_db = db.execute(query_usuario, {"email": current_user.get("email") or ""}).fetchone()
            if usuario_db:
                usuario_id = usuario_db.id
            else:
                usuario_id = 1 # fallback para demo
                
        # Realizamos la consulta uniendo reservas, horarios y canchas
        query = text("""
            ...
            ORDER BY r.fecha_reserva DESC, hc.hora_inicio DESC;
        """)
        
        resultados = db.execute(query, {"usuario_id": usuario_id}).mappings().all()
        
        reservas = []
        for row in resultados:
            row_dict = dict(row)
            row_dict["reserva_id"] = int(row_dict["reserva_id"])
            row_dict["total_pago"] = float(row_dict["total_pago"])
            row_dict["superficie"] = str(row_dict["superficie"])
            row_dict["estado"] = str(row_dict["estado"])
            row_dict["metodo_pago"] = str(row_dict["metodo_pago"]) if row_dict["metodo_pago"] else "efectivo"
            reservas.append(row_dict)
            
        return reservas
    except Exception as e:
        logger.error(f"Error obteniendo reservas de cliente: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error al obtener reservas: {str(e)}"
        )

# HU-15 - Obtener reservas agrupadas por día o semana para el administrador
@app.get("/api/admin/reservas", response_model=dict)
def obtener_reservas_admin(
    view: str = "week", # 'day' o 'week'
    db: Session = Depends(get_db),
    current_user: dict = Depends(require_admin)
):
    """
    HU-15: Panel de administración para visualizar el flujo total de reservas.
    Permite filtrar por 'day' o 'week'.
    """
    try:
        ...
        """)
        canchas_res = db.execute(query_canchas).mappings().all()
        canchas_list = []
        for c in canchas_res:
            canchas_list.append({
                "cancha_id": int(c["cancha_id"]),
                "nombre": str(c["nombre"]),
                "tipo_superficie": str(c["tipo_superficie"]),
                "precio_por_hora": float(c["precio_por_hora"])
            })
            
        # 2. Definir el filtro temporal para las reservas
        if view == "day":
            filtro_sql = "r.fecha_reserva = CURRENT_DATE"
        else: # view == "week"
            filtro_sql = "r.fecha_reserva >= DATE_TRUNC('week', CURRENT_DATE) AND r.fecha_reserva < DATE_TRUNC('week', CURRENT_DATE) + INTERVAL '7 days'"
            
        # 3. Obtener reservas activas
        query_reservas = text(f"""
            SELECT r.id AS reserva_id,
            ...
            JOIN horarios_cancha hc ON r.horario_cancha_id = hc.id
            WHERE {filtro_sql} AND r.estado != 'cancelada'
            ORDER BY r.fecha_reserva ASC, hc.hora_inicio ASC;
        """)
        
        reservas_res = db.execute(query_reservas).mappings().all()
        reservas_list = []
        for r in reservas_res:
        ...
        return {
            ...
            "canchas_disponibles": canchas_list,
            "reservas_activas": reservas_list
        }
    except Exception as e:
        logger.error(f"Error obteniendo reservas para administrador: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error al obtener reservas del panel de administración: {str(e)}"
        )
